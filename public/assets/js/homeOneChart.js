  ...
  var options = {
    series: [{
      ...
      data: ordersData.map(function(item) {
        return item.total_sales/1000; // Assuming each item has a 'total_sales' property
      })
    }],
    chart: {
      height: 264,
  ...
  var options = {
      series: [{
        ...
        data: weeklyOrderData.map(function(item) {
            return {
                x: item.day, // Assuming each item has a 'day' property
                y: item.total_amount // Assuming each item has a 'count' property
            };
        })
      }],
      chart: {
  ...
  // ================================ Total Subscriber bar chart End ================================ 
    ...
    var options = { 
      series: [transactionStats.pending_payment, transactionStats.completed_payment, transactionStats.failed_payment],
      colors: ['#FF9F29', '#327E82', '#FF9077'],
      labels: ['En attente', 'Validé', 'Échoué'] ,
      legend: {
          show: false 
      },
      chart: {
